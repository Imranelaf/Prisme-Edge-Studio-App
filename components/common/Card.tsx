  ...
  return (
    <>
      <div className='mx-auto px-5 py-8 text-center'>
        <h1 className="text-teal-800 font-extrabold uppercase text-4xl tracking-wide">Our Games</h1>
      </div>
      <div className="grid grid-cols-[repeat(auto-fill,_minmax(250px,_1fr))] grid-rows-[350px] gap-8 p-8">
        {games.map((game) => (
        ...
        {showAll && (
          <div className="relative border border-gray-300 rounded-lg overflow-hidden shadow transition-transform duration-200 cursor-pointer hover:transform hover:translate-x-1 hover:translate-y-[-5px] hover:shadow-lg">
            <Link href="/games" className="flex items-center justify-center h-full bg-yellow-400 text-2xl font-bold text-gray-800">
              Show ALL
            </Link>
          </div>
        )}
